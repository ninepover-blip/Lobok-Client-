    ...
    <nav className="sticky top-0 z-50 glass border-b border-white/5">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-[64px] flex items-center justify-between gap-4">
        <Link href="/" className="flex items-center gap-3">
          ...
          <span className="hidden sm:inline text-xs px-2 py-1 rounded-full bg-white/5 border border-white/10 text-white/60">HvH • MetaHvH</span>
        </Link>
          ...
          {me ? (
            <>
              <Link href={`/profile/${me.username}`} className="flex items-center gap-2 pl-1 pr-3 py-1 rounded-full glass">
                ...
                {me.role==="MODERATOR" && <span className="w-2 h-2 rounded-full bg-blue-500 shadow-[0_0_8px_#3b82f6]"></span>}
                {me.role==="ADMIN" && <span className="w-2 h-2 rounded-full bg-gradient-to-r from-violet-500 to-blue-500"></span>}
              </Link>
