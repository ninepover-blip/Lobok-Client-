...
// 1 free key per day globally, после подписки на Discord
export async function POST(req: NextRequest){
  const me = await getCurrentUser();
  if(!me) return NextResponse.json({error:"Войдите в аккаунт"},{status:401});
  const { discordConfirmed } = await req.json().catch(()=>({discordConfirmed:false}));
  ...
  const userClaimed = await prisma.freeKeyClaim.findFirst({ where:{ userId: me.id }});
  // allow only one free key per user ever? spec says 1 per day globally 1 person gets it, but let's allow 1 per user total to avoid abuse
  if(userClaimed) return NextResponse.json({error:"Ты уже получал фри-ключ"},{status:400});
  ...
  const k = await prisma.licenseKey.create({
    data:{
      key: keyStr, type:"FREE", status:"ACTIVE", durationDays:7, priceRub:0, priceUah:0,
